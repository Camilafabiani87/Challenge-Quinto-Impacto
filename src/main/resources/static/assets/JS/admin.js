const app = Vue.createApp({
  ...
  methods: {
    obtenerAlumnos(URL) {
      this.alumnos.forEach(alumnos => !this.cursos.includes(alumnos.courses)? this.cursos.push(alumnos.courses) : "")
...
cerrarSesion() {
  ...
  computed: {
    filtroDoble(){
      let primerFiltro =  this.backupAlumnos.filter(alumno => alumno.name.toLowerCase().includes(this.textoBuscar.toLowerCase()))
      console.log(this.cursosFiltrados);
      if (this.cursosFiltrados.length){
       let alumnosDobleFiltro = [];
       this.cursosFiltrados.forEach(curso => {
        primerFiltro.forEach(alumno => { if(alumno.courses.includes(curso) && !alumnosDobleFiltro.includes(alumno)){
